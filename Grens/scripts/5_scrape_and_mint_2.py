...
def extract_player_names_from_response(response, game):
    ...
    if game == "QW":
        ...
        # Quake 3 & Quake Live often have color codes (like ^7 etc.)
...
def main():
    ...
    while True:
        ...
        # Process each server and mint tokens in batches
        for server in serverPlayersLists:
            players = server["playerNames"]
            filtered_players = find_known_players(players, known_players)

            if not filtered_players:
                continue  # Skip if no registered players
            ...
            for chunk in chunks:
                try:
                    ...
                    print(f"Minting for chunk: {chunk}")

                    # Call mintForPlayersBatch
                    tx = wage_dapp.mintForPlayersBatch(
                        chunk,
                        {'from': validator_account, 'required_confs': 1} #, 'nonce': current_nonce}
                    )

                    # Capture the PlayerMinted events
                    if 'PlayerMinted' in tx.events:
                        for event_data in tx.events['PlayerMinted']:
                            gating_address = event_data['gatingAddress']
                            amount_to_player = event_data['amountToPlayer']
                            amount_to_validator = event_data['amountToValidator']

                            # Convert from 1e18 if you want to display in float
                            player_float = amount_to_player / 1e18
                            validator_float = amount_to_validator / 1e18

                            print(
                                f"PlayerMinted - gatingAddress: {gating_address}, "
                                f"playerCut: {player_float}, validatorCut: {validator_float}"
                            )

                    # Confirm and pause briefly
                    tx.wait(1)
                    time.sleep(1)
                    print(f"Minted for players. Tx hash: {tx.txid}")
